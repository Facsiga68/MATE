...
class PhoneBookModel extends Model
{
    ...
    public function Egyszeru_kereses($keres) {
        ...
        if ($keres!="") {
            $query .= "WHERE TRIM(COALESCE(szemely_nevelotag,'') || ' ' || COALESCE(szemely_csaladinev,'') || ' ' || COALESCE(szemely_keresztnev,'')) LIKE '%".
                $this->db->escapeLikeString($keres) . "%' ESCAPE '!'";        
        }
    ...
    public function Osszetett_kereses($keres){
        ...
        if ($keres!="") {
            $query .= "WHERE TRIM(COALESCE(szemely_nevelotag,'') || ' ' || COALESCE(szemely_csaladinev,'') || ' ' || COALESCE(szemely_keresztnev,'')) LIKE '%".
                $this->db->escapeLikeString($keres) . "%' ESCAPE '!' ".
                "  or szemely_telefon LIKE '%".$this->db->escapeLikeString($keres) . "%' ESCAPE '!' ".
                "  or szemely_mellek LIKE '%".$this->db->escapeLikeString($keres) . "%' ESCAPE '!' ".
                "  or szemely_emailceg LIKE '%".$this->db->escapeLikeString($keres) . "%' ESCAPE '!' ".
                "  or szemely__cim LIKE '%".$this->db->escapeLikeString($keres) . "%' ESCAPE '!' ".
                "  or s3.szervezet_megnevezes LIKE '%".$this->db->escapeLikeString($keres) . "%' ESCAPE '!' ".
                "  or s4.szadat_megnevezes LIKE '%".$this->db->escapeLikeString($keres) . "%' ESCAPE '!' ";
        }
        $data=$this->db->query($query);
        return $data->getResultArray();
    }   
